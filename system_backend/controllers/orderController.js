...
export const updateNewOrderStatus = async (req, res) => {
    ...
    const allowedStatuses = ['processing', 'completed', 'delivered', 'cancelled', 'returned'];
...
export const updatePaymentMethod = async (req, res) => {
    ...
    if (!order_id || !payment_method) {
        return res.status(400).json({ 
            error: 'Both order_id and payment_method are required' 
        });
    }

    // Validate the payment method is one of the allowed values
    const allowedMethods = ['full', 'advance'];
    ...
    try {
        ...
        // 3. If changing from advance to full, update amount_paid to total_amount
        if (currentOrder.payment_method === 'advance' && payment_method === 'full') {
            updateData.amount_paid = currentOrder.total_amount;
            updateData.payment_status = 'paid'; // Mark as fully paid
        }
        ...
        // 5. Get the updated order to return
        const [updatedOrder] = await pool.query(
            'SELECT * FROM orders WHERE order_id = ?',
            [order_id]
        );
        ...
        res.json({ 
            success: true,
            message: `Payment method updated to ${payment_method}`,
            order: updatedOrder[0]
        });
